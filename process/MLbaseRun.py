...
class FeatureSelector:
    ...
    def fit(self, Data, X, y, outdir):
        ...
        if self.name == 'XGBoost':
            classifier = XGBClassifier(learning_rate=self.params['learning_rate'], booster = 'gbtree', use_label_encoder = False, n_jobs = 2,
...
paramsPath = '/home/yincheng23/ADlasso_manuscript/Datasets/data/ML_parm_info/'
os.chdir(refPath)
dataSets = os.listdir(refPath)
dataSets = sorted(dataSets)
for dataSet in dataSets:
    #======================== Data loading=============================================
    if dataSet == 'LIBSVM_rs':
        continue
    ...
    os.chdir(paramsPath)
    os.chdir(dataSet)
    if not os.path.isfile("parm_info.txt"):
        continue
    if os.path.isfile('LASSO.txt'):
        continue
    BestPara = getParaSet()
