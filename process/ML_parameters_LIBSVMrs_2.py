...
FS = {}
params = {'XGBoost' : {'learning_rate': 0.05, 'n_estimators': 300, 'max_depth': 6,
                       'reg_alpha':1, 'reg_lambda': 1},
          'MI'      : {'n_neighbors' : 3}}
          #'reliefF' : {'k' : 3}}
models_FS = {'XGBoost' : FeatureSelector(name='XGBoost', params=params['XGBoost']),
             'MI'      : FeatureSelector(name='MI', params=params['MI'])}
             #'reliefF' : FeatureSelector(name='reliefF', params=params['reliefF'])}
tuned_parameters = {'XGBoost' : {'learning_rate': [0.05, 0.001], 'n_estimators': [50,100,300], 'max_depth': [2,6],
...
y_ = [1 if ix > 0 else 0 for ix in y]
y = np.array(y_)
prevalence = get_prevalence(X, np.arange(X.shape[0]))
max_num_feat = 618
k_fold = 5
...
foldpath = '/home/yincheng23/ADlasso_manuscript/Datasets/data/ML_parm_info/LIBSVM_rs/parm_info2.txt'
