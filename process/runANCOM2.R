...
for(dataSet in dataSets){
  ...
  #print(round(dim(ASV_table)[1]*dim(ASV_table)[2]/(6661*129),3))
  ...
  selected_list <- res
  selected_list_thr <- selected_list[selected_list$detected_0.6,]
  outPath_dataSet_selectedList <- paste0(outPath, dataSet, '/','ANCOM2', '.txt')
  write.table(selected_list_thr, file=outPath_dataSet_selectedList, quote=FALSE, sep="\t", col.names = F, row.names = F)
